...
public class AStarPathFinder implements PathFinder {
	...
	private Map<Integer, Map<Integer, Node>> nodes;
	//private Node[][] nodes;
	...
	public AStarPathFinder(TileBasedMap map, int maxSearchDistance, boolean allowDiagMovement) {
		this(map, maxSearchDistance, allowDiagMovement, new ManhattanHeuristic(maxSearchDistance));
	}
	...
	public Node getNodeAt(int i, int j){
		Node node;
		Map<Integer,Node> hash = nodes.get(i);
		if(hash == null){
	...
	public Path findPath(GameMover mover, int sx, int sy, int tx, int ty) {
		...
		Node sourceNode = getNodeAt(sx, sy);
		Node targetNode = getNodeAt(tx, ty);
			...
			for (int x=-1;x<2;x++) {
				for (int y=-1;y<2;y++) {
					...
					if (isValidLocation(mover,sx,sy,xp,yp)) {
						...
						Node neighbour = getNodeAt(xp, yp);
